...
/* Subject -> badge label. Extend this map to add more tags later. */
var TAG_LABELS = { Cnoc: "NoC", CRV5: "RISC-V", Csec: "SEC", Cfpga: "FPGA", CML: "ML", Cmpsoc: "MPSOC", Cfault: "FT", Cenergy: "LP", Csysmod: "SYS", Cmapping: "MAP" };
var TAG_ORDER  = ["Cnoc", "CRV5", "Csec", "Cfpga", "CML", "Cmpsoc", "Cfault", "Cenergy", "Csysmod", "Cmapping"];

function renderTagBadges() {
  ENTRIES.forEach(function (e) {
    var tags = TAG_ORDER.filter(function (s) { return e.subjects.indexOf(s) !== -1; });
    if (!tags.length) return;

    var target = findConfTextNode(e.el);

    var row = document.createElement("span");
    row.className = "tagRow";
    tags.forEach(function (s) {
      var badge = document.createElement("span");
      badge.className = "tag tag-" + s;
      badge.textContent = TAG_LABELS[s];
    ...
    if (target) {
      ...
      e.el.appendChild(row);   // no "In: <conf>, <year>" line found: fall back to end of entry
    }
  });
}

/* Locate the text node holding "In: <conf>, <year>" (may sit inside a nested
   <table> for award entries), so badges can be placed right after it. */
function findConfTextNode(li) {
  var walker = document.createTreeWalker(li, NodeFilter.SHOW_TEXT, null, false);
  var found = null, node;
  while ((node = walker.nextNode())) {
    if (/In:\s*[^,<]+,\s*\d{4}/.test(node.nodeValue)) found = node;
  }
  return found;
}
